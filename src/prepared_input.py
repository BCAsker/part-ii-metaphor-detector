...
class Prepared:
    ...
    def load_prepared(self):
        ...
        self.length = len(self.local_input_ids[0])
    ...
    def get_tensors(self, index=None):
        if index is None:
            return self.metaphor_labels, self.local_input_ids, self.local_attention_mask, self.local_token_type_ids
        else:
            return self.metaphor_labels[index], self.local_input_ids[index], self.local_attention_mask[index], \
